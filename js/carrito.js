...
function actualizarCarrito() {
  ...
  carrito.forEach((p, index) => {
    ...
    <td class="botonEliminarProduc"><button class="eliminar-producto" data-index="${index}" style="margin-left:10px; background:red; color:white;">Eliminar</button></td>
    `;
...
document.getElementById("finalizarCompra").addEventListener("click", () => {
  const mensaje = carrito.map(p => 
    `• ${p.nombre} x ${p.cantidad}%0AImagen: ${location.origin}/${p.imagen}`
  ).join('%0A%0A');

  const url = `https://wa.me/50688853533?text=Hola! Quiero comprar:%0A%0A${mensaje}`;
